...
def test_ci_workflow_uses_locked_canonical_quality_gate_for_supported_pythons():
    ...
    codecov_steps = [step for step in steps if step.get("uses", "").startswith("codecov/codecov-action")]
    assert len(codecov_steps) == 1
    assert codecov_steps[0]["with"]["token"] == "${{ secrets.CODECOV_TOKEN }}"  # noqa: S105
